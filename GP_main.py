...
for iii in range(inGP.numgen):
	...
	if iii == 0: # this case is the initial part.
		for i in range(len(pop)):
			temp = pop[i].tree
			[Substrate,polygons,centroid] = hp.get_all_para_for_hfss(temp) # get necessary parameters for genscript function.
	...
	if gen == 1:
		for i in range(len(pop)):
			...
			[pop[i].fitness, pop[i].ReturnLoss] = hpr.assignFitness(spec_nameDir_tab,gen) # update the fitness of each individual.
