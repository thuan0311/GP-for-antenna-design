...
def get_all_para_for_hfss(GP_pro):
    ...
    length = len(GP_pro.childs[1].valueofnode)
    parameters = [] # save all of coordinates of each polygon.

    for i in range(length):
        tempx = GP_pro.childs[1].valueofnode.values[i][0].exterior.xy[0]
        tempy = GP_pro.childs[1].valueofnode.values[i][0].exterior.xy[1]
        lenxy = len(tempx)
        tempPara = np.zeros((lenxy,3)) # save all the vertexes of a polygon.
        for i in range(lenxy):
            tempPara[i][0] = tempx[i]
            tempPara[i][1] = tempy[i]
    ...
    return [GP_pro.childs[0].valueofnode,parameters,centroid]
