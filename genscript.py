...
anten =  init.AnT()
sub = init.Sub()
def genscript(Substrate,polygons,centroid,tmpScriptFile,outFile,hfssFile):
	...
	length_W = Substrate[0] + addition_number # imply that is the width_ox
	length_L = Substrate[1] + addition_number # imply that is the length_oy
	...
	# solve. 
	hfss.hfssSolveSetup(fid, 'Solution1')
	hfss.hfssSaveProject(fid, hfssFile, True);
